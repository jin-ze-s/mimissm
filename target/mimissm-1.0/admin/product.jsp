...
<head>
    ...
    <script type="text/javascript">
        ...
        var lprice = '${vo.lprice}'
        var hprice = '${vo.hprice}'
        ...
        <c:choose>
            <c:when test="${pageInfo.list.size()!=0}">
                ...
                <div id="top">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/addproduct.jsp?page=${pageInfo.pageNum}">
...
<%--                    TODO--%>
                    <input type="button" class="btn btn-warning" id="btn2"
                           value="批量删除" onclick="deleteBatch(${pageInfo.pageNum})">
                </div>
                <!--显示分页后的商品-->
                <div id="middle">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.pId},${pageInfo.pageNum})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${p.pId},${pageInfo.pageNum})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
                            ...
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    <li>
                                        ...
                                        <a href="javascript:ajaxsplit(${pageInfo.pageNum-1})" aria-label="Previous">
                                    ...
                                    <li>
                                        ...
                                        <a href="javascript:ajaxsplit(${pageInfo.pageNum+1})" aria-label="Next">
                                            <span aria-hidden="true">»</span></a>
                                    </li>
                                    <li style=" margin-left:150px;color: #0e90d2;height: 35px; line-height: 35px;">总共&nbsp;&nbsp;&nbsp;<font
                                            style="color:orange;">${pageInfo.pages}</font>&nbsp;&nbsp;&nbsp;页&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    function deleteBatch(pageNo) {
        var deleteEmps=new Array();
        $("input[name=ck]").each(function () {
            if (this.checked){
                deleteEmps.push($(this).val())
            }
        })
        if (deleteEmps.length===0){
            alert("请选择要删除的商品")
            return false
        }
        if (confirm("确认删除这"+deleteEmps.length+"件商品吗？")){
        $.ajax({
            url: "${pageContext.request.contextPath}/prod/batchDelete.action",
            type: "POST",
            data:"pids="+JSON.stringify(deleteEmps),
    ...
    function del(pid,pageNum) {
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            window.location="${pageContext.request.contextPath}/prod/delete.action?pid="+pid+"&page="+pageNum
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        //异步ajax分页请求
        $.ajax({
        url:"${pageContext.request.contextPath}/prod/splitPage.action",
